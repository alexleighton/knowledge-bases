module Cmd = Cmdliner.Cmd
module Term = Cmdliner.Term
module Arg = Cmdliner.Arg

module Common = Cmdline_common
module Service = Kbases.Service.Kb_service
module Relation = Kbases.Data.Relation
module Relation_kind = Kbases.Data.Relation_kind
module Identifier = Kbases.Data.Identifier

let run source depends_on related_to uni bi =
  let target, kind, bidirectional =
    match depends_on, related_to, uni, bi with
    | Some tgt, None, None, None -> (tgt, "depends-on", false)
    | None, Some tgt, None, None -> (tgt, "related-to", true)
    | None, None, Some (k, tgt), None -> (tgt, k, false)
    | None, None, None, Some (k, tgt) -> (tgt, k, true)
    | _ ->
        Common.exit_with
          "exactly one of --depends-on, --related-to, --uni, or --bi is required"
  in
  let ctx = App_context.init () in
  Fun.protect ~finally:(fun () -> App_context.close ctx) (fun () ->
    match Service.relate (App_context.service ctx) ~source ~target
            ~kind ~bidirectional with
    | Ok { Service.relation = rel; source_niceid; target_niceid } ->
        Printf.printf "Related: %s %s %s (%s)\n"
          (Identifier.to_string source_niceid)
          (Relation_kind.to_string (Relation.kind rel))
          (Identifier.to_string target_niceid)
          (if Relation.is_bidirectional rel then "bidirectional"
           else "unidirectional")
    | Error err -> Common.exit_with (Common.service_error_msg err))

let source_arg =
  let doc = "Niceid (e.g. kb-0) or TypeId of the source item." in
  Arg.(required & pos 0 (some string) None & info [] ~docv:"SOURCE" ~doc)

let depends_on_opt =
  let doc = "Target identifier. Creates a unidirectional depends-on relation." in
  Arg.(value & opt (some string) None & info [ "depends-on" ] ~docv:"TARGET" ~doc)

let related_to_opt =
  let doc = "Target identifier. Creates a bidirectional related-to relation." in
  Arg.(value & opt (some string) None & info [ "related-to" ] ~docv:"TARGET" ~doc)

let uni_opt =
  let doc =
    "User-defined unidirectional relation. Value is KIND,TARGET (e.g. designed-by,kb-1)."
  in
  Arg.(value & opt (some (pair ~sep:',' string string)) None
    & info [ "uni" ] ~docv:"KIND,TARGET" ~doc)

let bi_opt =
  let doc =
    "User-defined bidirectional relation. Value is KIND,TARGET (e.g. reviews,kb-1)."
  in
  Arg.(value & opt (some (pair ~sep:',' string string)) None
    & info [ "bi" ] ~docv:"KIND,TARGET" ~doc)

let cmd_man = [
  `S "EXAMPLES";
  `P "bs relate kb-0 --depends-on kb-1";
  `P "bs relate kb-0 --related-to kb-1";
  `P "bs relate kb-0 --uni designed-by,kb-1";
  `P "bs relate kb-0 --bi reviews,kb-1";
]

let cmd_info = Cmd.info "relate" ~doc:"Create a relation between two items." ~man:cmd_man

let cmd =
  let term =
    Term.(const run $ source_arg $ depends_on_opt $ related_to_opt $ uni_opt $ bi_opt)
  in
  Cmd.v cmd_info term
