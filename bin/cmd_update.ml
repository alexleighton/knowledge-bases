...
let run identifier status title content_opt json =
  ...
  Fun.protect ~finally:(fun () -> App_context.close ctx) (fun () ->
    ...
    | Ok item ->
        let niceid = Identifier.to_string (Item.niceid item) in
        let entity_type = Item.entity_type item in
        if json then
          Common.print_json (`Assoc [
            "ok", `Bool true; "action", `String "updated";
            "type", `String entity_type; "niceid", `String niceid;
          ])
        else
