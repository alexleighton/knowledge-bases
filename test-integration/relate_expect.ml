...
let%expect_test "bs relate --json error target not found" =
  Helper.with_git_root (fun dir ->
    Helper.init_kb dir;
    ignore (Helper.run_bs ~dir ~stdin:"Body" ["add"; "todo"; "Source"]);
...
let%expect_test "bs relate --json error no relation flags" =
  Helper.with_git_root (fun dir ->
    Helper.init_kb dir;
...
let%expect_test "bs relate --related-to with --blocking" =
  Helper.with_git_root (fun dir ->
    Helper.init_kb dir;
    ignore (Helper.run_bs ~dir ~stdin:"Body" ["add"; "todo"; "Source"]);
    ignore (Helper.run_bs ~dir ~stdin:"Body" ["add"; "todo"; "Target"]);
    let result = Helper.run_bs ~dir
      ["relate"; "kb-0"; "--related-to"; "kb-1"; "--blocking"] in
    Helper.print_result ~dir result);
  [%expect {|
    [exit 0]
    Related: kb-0 related-to kb-1 (bidirectional)
...
let%expect_test "bs relate --uni with --blocking" =
  Helper.with_git_root (fun dir ->
    Helper.init_kb dir;
    ignore (Helper.run_bs ~dir ~stdin:"Body" ["add"; "todo"; "Source"]);
    ignore (Helper.run_bs ~dir ~stdin:"Body" ["add"; "note"; "Target"]);
    let result = Helper.run_bs ~dir
      ["relate"; "kb-0"; "--uni"; "tracks,kb-1"; "--blocking"] in
    Helper.print_result ~dir result);
  [%expect {|
    [exit 0]
    Related: kb-0 tracks kb-1 (unidirectional)
  |}]
